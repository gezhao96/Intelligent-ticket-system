...
def test_ai_success_is_only_a_proposal(client):
    ...
    body = response.json()
    assert body["ai_category"] == "NETWORK"
    assert body["ai_priority"] == "P2"
    assert body["review_status"] == "PENDING"
    assert body["final_category"] == "OTHER"
    assert body["final_priority"] == "P3"
...
def test_prompt_injection_content_is_data_and_not_applied(client):
    ...
    set_mock_client(
        client,
        ai_json(category="HARDWARE_OFFICE", priority="P2", injection_detected=True),
        captured.append,
    )
    try:
        response = client.post(f"/tickets/{ticket['id']}/ai-analysis")
    ...
    assert body["ai_category"] == "HARDWARE_OFFICE"
...
def test_ai_invalid_response_fails_without_changing_final_fields(client):
    ...
    assert stored["final_category"] == "OTHER"
    assert stored["final_priority"] == "P3"
...
def test_ai_auth_failure_keeps_ticket_core_workflow_available(client):
    ...
    transition = client.patch(f"/tickets/{ticket['id']}/status", json={"final_status": "IN_PROGRESS", "actor": "alice"})
...
def test_human_confirm_modify_and_reject_are_traceable(client):
    ...
    assert confirmed.json()["final_category"] == "NETWORK"
    ...
    assert modified.json()["ai_category"] == "NETWORK"
    assert modified.json()["final_category"] == "SOFTWARE_INCIDENT"
    ...
    assert rejected.json()["final_category"] == "OTHER"
